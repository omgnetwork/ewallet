defmodule KuberaAdmin.V1.UserAuthPlug do
  @moduledoc """
  ...
  KuberaDB.Key's `access_key` and `secret_key` should not be used.
  """
  import Plug.Conn
  import KuberaAdmin.V1.ErrorHandler
  alias KuberaDB.Helpers.Crypto
  alias KuberaDB.{AuthToken, User}
  alias KuberaAdmin.V1.ClientAuthPlug
  ...
  defp authenticate_token(conn) do
    ...
    case AuthToken.authenticate(user_id, auth_token, :kubera_admin) do
      false ->
        conn
        |> assign(:authenticated, false)
  ...
  def expire_token(conn) do
    ...
    AuthToken.expire(token_string, :kubera_admin)
