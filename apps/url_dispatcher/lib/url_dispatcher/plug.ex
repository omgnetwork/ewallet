defmodule UrlDispatcher.Plug do
  @moduledoc false
  import Plug.Conn, only: [resp: 3, halt: 1, send_resp: 3, put_status: 2]
  ...
  defp handle_request("/api", conn), do: EWalletAPI.Endpoint.call(conn, [])
  defp handle_request("/api/" <> _, conn), do: EWalletAPI.Endpoint.call(conn, [])
  defp handle_request("/admin/api", conn), do: AdminAPI.Endpoint.call(conn, [])
  ...
  defp handle_request("/public/" <> _, conn) do
    ...
    case Static.call(conn, opts) do
      ...
      _ ->
        conn
        |> send_resp(404, "The url could not be resolved.")
        |> halt()
    end
  end
