...
defmodule EWallet.BlockchainWalletPolicy do
  @moduledoc """
  The authorization policy for blockchain wallets.
  """
  ...
  def authorize(:create, actor, %{"account_uuid" => account_uuid})
    ...
    Bouncer.bounce(actor, %Permission{
      action: :create,
      target: %BlockchainWallet{account_uuid: account_uuid}
    })
  end
  ...
  def authorize(action, actor, target) do
    PolicyHelper.authorize(action, actor, :blockchain_wallets, BlockchainWallet, target)
  end
end
