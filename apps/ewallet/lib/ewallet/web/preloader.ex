...
defmodule EWallet.Web.Preloader do
  ...
  """
  @spec preload_one(map(), atom() | [atom()], [atom()]) :: {:ok, Ecto.Schema.t()} | {:error, nil}
  def preload_one(record, preloads, opts \\ []) when is_map(record) do
    case Repo.preload(record, List.wrap(preloads), opts) do
      nil -> {:error, nil}
      %{} = result -> {:ok, result}
    end
  end
