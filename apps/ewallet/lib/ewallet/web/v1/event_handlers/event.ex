defmodule EWallet.Web.V1.Event do
  ...
  def transaction_request_topic(topics, external_id) do
    topics ++ ["transaction_request:#{external_id}"]
  end

  def transaction_consumption_topic(topics, external_id) do
    topics ++ ["transaction_consumption:#{external_id}"]
  end
  ...
  def user_topic(topics, user_id) do
    case user_id do
      ...
      user_id ->
        ...
        topics ++ ["user:#{user.provider_user_id}", "user:#{user.external_id}"]
    end
  end
  ...
  def account_topic(topics, account_id) do
    case account_id do
      ...
      id ->
        ...
        topics ++ ["account:#{account.external_id}"]
    end
  end
