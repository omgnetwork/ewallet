...
defmodule EWallet.UserAuthTokenTest do
  ...
  describe "authenticate" do
    test "return user if the given user has enabled 2FA and the given token existed" do
      ...
      assert UserAuthToken.authenticate(user, auth_token.token, :admin_api) == user
    end
    ...
    test "return PreAuthToken if the given user has enabled 2FA and the given token existed" do
      ...
      assert pre_auth_token = UserAuthToken.authenticate(user, auth_token.token, :admin_api)
      assert pre_auth_token.token == auth_token.token
    end

    test "return false if the given user has enabled 2FA but the given token didn't exist" do
      user = insert(:user, %{enabled_2fa_at: ~N[2000-02-02 20:02:02], originator: nil})

      assert UserAuthToken.authenticate(user, "1234", :admin_api) == false
    end

    test "return false if the given token didn't exist" do
    ...
    test "return false if the given user is nil" do
      ...
      assert UserAuthToken.authenticate(user, nil, :admin_api) == false
    end

    test "return false if the given token is nil" do
      assert UserAuthToken.authenticate(nil, "1234", :admin_api) == false
    end
  end

  describe "generate" do
    test "return {:error, :invalid_parameter} if the given user is nil" do
      assert UserAuthToken.generate(nil, :admin_api, %System{}) == {:error, :invalid_parameter}
    end
    ...
    test "return {:ok, PreAuthToken} if the given user has enabled two-factor authentication" do
      ...
      assert {:ok, %PreAuthToken{}} = UserAuthToken.generate(user, :admin_api, %System{})
    end
    ...
    test "return {:ok, AuthToken} if the given user has not enabled two-factor authentication" do
      ...
      assert {:ok, %AuthToken{}} = UserAuthToken.generate(user, :admin_api, %System{})
    end
  end
end
