...
defmodule EWallet.BlockchainDepositWalletGateTest do
  ...
  describe "refresh_balances/3" do
    test "updates the deposit wallet balances with the latest blockchain state" do
      ...
      _ = insert(:blockchain_deposit_wallet_balance, blockchain_deposit_wallet: wallet, amount: 10, token: token_1)
      _ = insert(:blockchain_deposit_wallet_balance, blockchain_deposit_wallet: wallet, amount: 20, token: token_2)
      ...
      {res, data} = BlockchainDepositWalletGate.refresh_balances(wallet.address, "dumb", [token_1, token_2])
      assert res == :ok
      assert [{:ok, _}, {:ok, _}] = data

      # The dumb adapter hard codes the amount to 123
      wallet = BlockchainDepositWallet.reload_balances(wallet)
      assert Enum.any?(wallet.balances, fn b -> b.token_uuid == token_1.uuid && b.amount == 123 end)
      assert Enum.any?(wallet.balances, fn b -> b.token_uuid == token_2.uuid && b.amount == 123 end)
    end
  end
end
