defmodule EWallet.Web.V1.TransactionConsumptionEventHandlerTest do
  ...
  describe "broadcast/1" do
    test "broadcasts the 'transaction_consumption_approved' event" do
      ...
      res = TransactionConsumptionEventHandler.broadcast(:transaction_consumption_approved,
      ...
      |> Enum.each(fn topic ->
        assert Enum.member?(mapped, {"transaction_consumption_approved", topic})
      end)

      :ok = TestEndpoint.stop()
    end

    test "broadcasts the 'transaction_consumption_rejected' event" do
      {:ok, _} = TestEndpoint.start_link()
      ...
      res = TransactionConsumptionEventHandler.broadcast(:transaction_consumption_rejected,
                                                         %{consumption: consumption})
      events = TestEndpoint.get_events()
      ...
      |> Enum.each(fn topic ->
        assert Enum.member?(mapped, {"transaction_consumption_rejected", topic})
      end)
