...
defmodule EWallet.Web.MatchAnyQueryTest do
  use EWallet.DBCase, async: true
  import Ecto.Query
  import EWalletDB.Factory
  alias EWallet.Web.MatchAnyQuery
  ...
  describe "do_filter/5 with uuid field type" do
    test "supports 'eq' comparator" do
      ...
      result =
        false
        |> MatchAnyQuery.do_filter(:uuid, :uuid, "eq", user_2.uuid)
        |> on_all(User)
    ...
    test "supports 'neq' comparator" do
      ...
      result =
        false
        |> MatchAnyQuery.do_filter(:uuid, :uuid, "neq", user_2.uuid)
        |> on_all(User)
    ...
    test "supports 'contains' comparator" do
      ...
      result =
        false
        |> MatchAnyQuery.do_filter(:uuid, :uuid, "contains", partial_uuid)
        |> on_all(User)
    ...
    test "supports 'starts_with' comparator" do
      ...
      result =
        false
        |> MatchAnyQuery.do_filter(:uuid, :uuid, "starts_with", uuid_head)
        |> on_all(User)
