defmodule EWallet.Web.InviterTest do
  ...
  describe "invite_admin/5" do
    test "sends email and returns the invite if successful" do
      ...
      {res, invite} =
        Inviter.invite_admin("test@example.com", account, role, @redirect_url, InviteEmail)
    ...
    test "returns :invalid_email error if email is invalid" do
      ...
      {res, error} = Inviter.invite_admin(email, account, role, @redirect_url, InviteEmail)
    ...
    test "returns :user_already_active error if user is already active" do
      ...
      {res, error} =
        Inviter.invite_admin("activeuser@example.com", account, role, @redirect_url, InviteEmail)
